...
//
// OJO CON falAncho Y falVuelo: estan en pixeles absolutos, no en proporcion,
// asi que van ATADOS al falAncho base de romi-pose.js. Cuando ese bajo de 46
// a 37 (para que el vestido dejara de ser el doble de ancho que la cabeza),
// estos numeros seguian en 47-54 y la falda se volvia a ensanchar hasta la
// campana de antes EN CUANTO ELLA SE MOVIA. Reescalados todos por 37/46.
//
...
export const POSES = {
  ...
  idle: [
    ...
    // la cabeza le sigue con RETARDO (llega un fotograma despues, como una
    // cabeza de verdad sobre un cuello), los hombros acompañan y hay un
  ...
  run: [
    // EL REBOTE, medido y subido. Tenia 5 px de recorrido de cadera (111 a
    // 116) y las dos mitades del ciclo eran IDENTICAS en cadera e
    // inclinacion: por eso se veia como deslizarse y no como correr. Una
    // carrera de verdad bota, y las dos zancadas no son iguales -- la del pie
    // dominante pisa mas fuerte.
    //
    // Ahora el rebote es de 11 px y la segunda mitad rebota algo menos que la
    // primera, que es lo que le da el "tas-ta, tas-ta" en vez del "ta-ta-ta"
    // de metronomo. La inclinacion tambien respira: de 0.16 a 0.30.
    //
  ...
  jump: [
    // 1. IMPULSO. La ANTICIPACION: se agacha MAS de lo que parece necesario y
    ...
    pose({ cadY: 127, torY: -24, incl: 0.22, falVuelo: 2, falAlto: 50, falAncho: 44,
           ...
           falGajos: tela(0.0, 0.45), falBorde: borde(0.0, 0.5) }),
    ...
    pose({ cadY: 131, torY: -22, incl: 0.26, falVuelo: -5, falAlto: 46, falAncho: 47,
           ...
           falGajos: tela(5.4, 0.6), falBorde: borde(5.4, 0.66) }),
  ],
  ...
  //
  // Y la mano IZQUIERDA va al centro del escudo, como en block: manI sale de
  // escX + 15 (los hombros estan en -15 y +15 del eje). Medido, aqui estaba a
  // 26-27 px del centro de un escudo que mide 17 de semiancho, o sea el puño
