...
**Derrota:** el sexto fallo de UNA palabra. No hay vidas aparte: los globos que quedan al ganar arrastran a la palabra siguiente (+3, tope 6), así que la altura del muñeco ES el estado de la partida, sin ningún contador. La partida termina en el primer chapuzón con `ctx.gameOver(Math.floor(puntaje))`.
...
5. **PERDER:** chapuzón, EMPAPADO, 'SE MOJO... ERA:' con la palabra en naranja. SOLA → `ctx.gameOver(Math.floor(puntaje))` a los 4.3 s (o toque desde 1.5 s): la pantalla de fin es del motor, hereda el formato vertical y dice 'ERES INCREIBLE ROMINA' si es récord; 'TOCA PARA JUGAR' vuelve a `init()` → ENTRADA. A DOS → tarjeta.
6. **A DOS, ESCRIBIR:** pista 'ESCRIBE LA PALABRA SECRETA' (escala 2, cian) y en la línea de mensajes dos botones de 240x44: 'ESPACIO' (x 21..261) y 'LISTO' (x 279..519, apagado hasta 3 letras). Las casillas muestran lo escrito (máx 14 caracteres, mín 3 letras, solo A-Z, Ñ y espacio; un solo espacio, nunca al principio ni al final) con cursor parpadeante (500 ms). Teclado en modo escribir: la séptima casilla de la cuarta fila es BORRAR (icono ⌫ con dos trazos). El muñeco NO MIRA (se tapa los ojos y se asoma cada 3 s).
